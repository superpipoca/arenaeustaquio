...
export function InlineTokenTradePanel({
  ...
  // ======================================================
  // ✅ WRAPPER DE INVOKE COM TOKEN DO CLERK
  // ======================================================
  const invokeArenaOrThrow = useCallback(
    async <T = any,>(body: any): Promise<T> => {
      if (!isSignedIn) {
        throw new Error("UNAUTHENTICATED");
      }

      // tenta template "supabase" se existir; senão cai no padrão
      const clerkToken =
      ...
      if (!clerkToken) {
        throw new Error(
          "Sessão expirada ou sem token de login. Faça login novamente."
        );
      }
      ...
      const { data, error } = await supabase.functions.invoke("arena-trade", {
        body: {
          ...body,
          // opcional: manda no body pra debug
      ...
      if (error) {
        ...
        if (ctxErr instanceof Response) {
          responseBody = await tryReadResponseBody(ctxErr);
          console.log("error.context.status:", ctxErr.status);
          console.log("error.context.body (read):", responseBody);
        } else {
          try {
            const safeCtx = {
              ...ctxErr,
              headers: redactHeaders(ctxErr?.headers),
              responseHeaders: redactHeaders(ctxErr?.responseHeaders),
            };
            console.log("error.context (safe):\n", safeStringify(safeCtx));
          } catch (e) {
            console.warn("[arena-trade] failed to stringify error.context", e);
        ...
        const serverMsg = responseBody || extractInvokeErrorMessage(error);
        throw new Error(serverMsg || error.message || "Falha na Edge Function.");
      }
  ...
  if (!isLoaded) {
    return (
      <section className="inline-trade-panel">
        <div className="inline-feedback">Carregando sessão...</div>
      </section>
    );
  }
  ...
  }, [token.id, range, ctx?.market.price_current, token.price]);
    ...
    >
      ...
      <div className="inline-book">
        ...
        {loadingBook ? (
          ...
          <div className="inline-feedback inline-feedback--error">{bookError}</div>
        ) : (
          <div className="inline-book-grid">
            <div className="inline-book-col">
              <div className="inline-book-col-title">BIDs (compra)</div>
          ...
          <>
            ...
            {isMarket ? (
              <div className="inline-est">
                <span>Estimativa de recebimento</span>
                <strong>
                  {buyEst ? `${formatCoin(buyEst.coinOut)} ${token.ticker}` : "--"}
                </strong>
                <small>
                  Mínimo com slippage ({parsedSlippage}%):{" "}
              ...
              <div className="inline-est">
                ...
                <small>
                  Custo total:{" "}
                  {limitPreview ? formatCurrency(limitPreview.costBase) : "--"}
                </small>
              </div>
            )}
          </>
          ...
          <>
            ...
            {isMarket ? (
              ...
              <div className="inline-est">
                <span>Preview LIMIT</span>
                <strong>
                  {limitPreview ? formatCurrency(limitPreview.grossBase) : "--"}
                </strong>
                <small>
                  Recebimento bruto:{" "}
                  {limitPreview ? formatCurrency(limitPreview.grossBase) : "--"}
